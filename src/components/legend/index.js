...
const Legend = ({ position, legends, metricAliases, truncateLabel, formatData, typography }) => {
  let objPosition = {}
    ...
    <>
      ...
      >
        {legends.map(({ type, ...legendProps }) => (
          <LegendItem
            key={type}
            legendItemProps={{ type, metricAliases, truncateLabel, formatData, ...legendProps }}
          />
        ))}
      </LegendContainer>
    </>
