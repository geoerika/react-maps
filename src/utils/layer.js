...
export const setFinalLayerDataProperty = ({
  ...
  // case for text layer
  if (value.title) {
    return d => getLabel(d)({ value, dataPropertyAccessor, formatDataKey, formatDataValue })
  }
 ...
 */
const getLabel = d => ({ value, dataPropertyAccessor, formatDataKey, formatDataValue }) => {
  let labelValues = ''
  ...
  const getLabelValue = ({ valueKey }) =>
    `\n${formatDataKey(valueKey)}: ${getFormatLabelValue(d)({ valueKey, labelKeyValue, formatDataValue })}`

  labelValues = value?.valueKeys?.reduce((acc, valueKey) => acc + getLabelValue({ valueKey }), '')

  return `${formatDataKey(value.title)}: ${dataPropertyAccessor(d)[value.title]}${labelValues.length ? labelValues : ''}`
}
